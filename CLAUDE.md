...
- 2단계: Catalyst (Mac)
- 3단계: Android (Kotlin + Jetpack Compose). 이전엔 Java + XML로 적혀 있었으나 SwiftUI declarative paradigm 1:1 매핑 + Google 공식 권장 스택 기준으로 변경 (2026-06-02).
...
- **완료 occurrence skip**: `syncNotifications`가 `finishedOccurrences`(done/failed RC stamp) 집합을 만들어 그 occurrence는 재등록 skip. 미리 완료(future occurrence) 시 `cancelNotifications(forOccurrence:)`로 즉시 취소 + refresh 시 재등록 방지 2중 가드.
- **CloudKit import 후 재조정**: `RootView`가 `.NSPersistentStoreRemoteChange` 구독(1.5s debounce) → `refreshAllRoutineNotifications` 재실행. 재설치/기기변경/**다른 기기에서 완료** 시 완료 RC가 늦게 import돼도 예약 알림(이미 완료된 occurrence)을 취소. import 전에 등록된 알림이 그대로 fire되던 버그 fix (2026-06-04). remote change는 post 설정만 돼 있었고 구독이 없어 누락됐었음.
...
## 최근 완료 (2026-06-02 회사 세션 — 빈화면 마감 + 저장 chain 정리 + onboarding 진행 중)

### 빈화면 (Case A/B/C) — 마감
- ArchiveView/ListView: 활성 0 케이스 → List 안 첫 Section의 EmptyStateView row + 완료 섹션 토글 ON 시 finished row 같이 노출. 토글 OFF면 큰 메시지만.
- TodayView: `isGenuineEmpty` = `activeNtds.isEmpty && activeActivities.isEmpty` 기준 (finished 제외). topPadding은 month=0, day=40.
- Case B (필터/검색 결과 empty): `filteredEmptyRow` inline (필터 해제 버튼).
- 자세한 정책은 위 "빈 화면 작업 상태" 섹션 참조.

### AddItemView — target 변경 안내 + 저장 chain 재정렬
- **`RoutineCompletion.targetSnapshot: Double?`** — 활동/집중 RC 생성/업데이트 시 현재 target snapshot 저장. done/failed RC는 snapshot 보존 → 사용자가 target 올려도 과거 완료 RC가 풀리지 않음.
  - `Item.effectiveTargetValue(on:)` — snapshot 우선 lookup. ItemRow / FocusSessionView / Widget / ActivityHistoryView / Item+HealthKitSync / Item+FocusSession 모두 이걸 통해 진행률·done 판정.
  - `RootView.task`에서 1회 backfill 마이그레이션 (UserDefaults flag `migrated.rc_target_snapshot`).
- **편집 모드 + 활동/집중 + target 변경** 저장 시 안내 alert ("변경된 목표 수치는 새로 시작하는 활동부터 적용됩니다"). 활동/집중 메시지 분기.
- **저장 chain 순서 재정렬** — 외부 이동(시스템 prompt / 설정 앱 / sheet) 가능 단계 먼저, 정보성 confirm 단계 뒤. 사용자가 외부 이동 후 돌아와도 정보성 dialog 중복 노출 회피:
  1. `canSave` (누락 안내 — terminate)
  2. HK 권한 요청 (활동 auto source — 미결정시만 시스템 prompt)
  3. 알림 권한 거부 경고 (설정 앱 이동 가능)
  4. 1회성 목표 confirmation (반복 sheet 이동 가능)
  5. target 변경 안내 (정보성 확인)
  6. `save()`
- `hasAlertConfigured` 활동 분기 추가 — `notifyOnGoalReached` toggle도 alert로 카운트 (default ON이라 사용자가 안 만져도 활성). HK BG handler가 target 도달 시 fire하므로 권한 필요.

### Onboarding 진행 중 (아직 폴리시 중 — 집에서 이어할 예정)
- **새 파일**: [Views/OnboardingView.swift](MyDays/MyDays/Views/OnboardingView.swift)
- **트리거**: `@AppStorage(UIStateKey.onboardingShown) var onboardingShown: Bool = false` — MyDaysApp의 WindowGroup에서 분기. false면 OnboardingView, true면 RootView.
- **구성** — 5 페이지 + 상단 건너뛰기 + 하단 dot + CTA 버튼:
  1. 할일 (반복 + 체크리스트) — illustration: row 2개 (반복 todo + 체크리스트 펼친 todo with 4 sub-item). 문구 사용자 정할 예정 (현재 placeholder).
  2. 4-type 목표 — 4 chip + progress row 2개.
  3. 오늘 + 위젯 — 오늘 화면 미니(weekstrip + 섹션) + 홈 small + 잠금 circular.
  4. Inbox — 빈 list + QuickEntryBar capsule (mic + plus).
  5. 권한 안내 — 알림/건강/마이크 3종 정보. 실제 prompt X (각 권한은 기능 첫 사용 시점 contextual).
- **단순화된 일러스트** 방식 — 실제 ItemRow/위젯 컴포넌트 재활용 X. PlaceholderBar + 색 chip + SF Symbol 조합.
- **Localization**: `onboarding.feature.{todo,goal,today,inbox}.{title,body}` + `onboarding.{title,notification,healthkit,microphone}.{title,body}` + `onboarding.{cta,next,skip,footnote}`. ko/en 둘 다.
- **남은 작업** (집에서):
  - 페이지 1 문구 확정 (현재 "할일을 자세히 관리하세요" / "기간, 반복, 체크리스트까지..." placeholder).
  - 다른 페이지 문구 검토.
  - 시뮬레이터/실기기에서 시각 확인 — 일러스트 크기/간격 조정, 폰트 확인.
  - Settings에서 onboarding 재진입 옵션 추가할지 결정 (Dev section 토글).
- **테스트 리셋**: `xcrun simctl uninstall booted io.snapplay.MyDays` 또는 앱 삭제.

### 기타 폴리시 (이번 세션)
- Archive 탭 영문 라벨: "Archive" → **"Inbox"** (한글 "보관함" 유지). `tab.archive` / `archive.title` / `archive.empty` 영문값 갈아끼움. 코드의 `ArchiveView` 클래스명은 미변경 — v2+에서 리네임 고려.
- HealthKit 안내 문구 영문 "flights" → "stairs" (앱 다른 곳 `activity_source.flights = "Stairs" / "계단"`과 통일).
- AddItemView activityRow 표시 정리: 활동/집중 record도 "달성"/"포기" 라벨 표시 + 진행도 "75/100" 형태로 comment 위치에 표시. `activityRowTarget(record)` 헬퍼는 RC.targetSnapshot 우선.

## 최근 완료 (2026-06-04 집 세션 — MissionRow 리팩토링 + YearGridView + WeekStrip 통합)

### MissionRow 리팩토링 (3 phase) — 멘탈 모델 ↔ 코드 구조 매칭
이전: ItemRow가 Todo + activity/focus/habit, NTDRow가 NTD 전용 → 4-type 목표가 두 row 클래스에 흩어짐.
이후: **MissionRow가 4-type 목표 통합 / ItemRow는 Todo 전용** — "할일 vs 목표" 멘탈 모델과 코드 매칭.

**Phase 1**: 신규 파일 `Views/MissionRow.swift` (~530줄). NTDRow + ItemRow goal 분기 흡수. 자체 포함 helper.
- API: `MissionRow(item:, occurrenceDate:, displayedDate:, compactMode:, mode:)`
- 내부 kind switch — NTD는 `TimelineView + AdaptiveCountdownSchedule`, activity/focus/habit은 정적 rendering
- 공통: 4-state goalLeadingIcon(legacy clock fallback 포함), 제목 앞 categoryBar(iconColorHex 우선), statusIcons(streak/bell/repeat/NTD clock), picker/cancel mode 대응
- kind별 trailing: NTD progress capsule + (x), 활동 (+)/heart, 집중 ▶, 습관 체크
- 액션: 습관 toggle / NTD 포기 / context save 자체 구현
- `mode: .today / .list` — list 모드에선 trailing 숨김, D-day/카운트다운/완료 일시 라벨 노출

**Phase 2**: 호출 위치 교체. TodayView.goalRow / ListView.rowButton + 검색 / ArchiveView.archiveRow + 완료 섹션 — 모두 `isGoal` 분기로 MissionRow ↔ ItemRow.

**Phase 3**: 청소.
- **`Views/NTDRow.swift` 통째 삭제** (~600줄). `NTDRow.completionTimeText` static helper만 `MissionRow.completionTimeText`로 이동. ItemRow의 2곳 호출처 갱신.
- **ItemRow에서 goal dead code 일괄 제거**: `isGoal`/`isHabit`/`isActivity`/`isFocus`/`isNTD`, `goalLeadingIcon`, `goalIconRenderStyle`, `habitTrailingCheck`, `activityTrailingProgress`, `focusTrailingProgress`, `goalAccentColor`, `activityOccurrenceDate`, `focusOccurrenceDate`, `ntdIconStyle`, `ntdStatusIcon`, `ntdListLabel`, `ntdDurationText`, `formatDuration`, `presentFocusSession` state + fullScreenCover.
- 단순화: `leadingControl`/`categoryBarColor`/`statusIcons`/`dueDayLabel`/`toggleDone` 모두 Todo 한정 분기로 축소.
- ItemRow: 1251 → 801줄 (-434줄). NTDRow: 599 → 0줄. 총 -1033줄.

**중복 제거** (Step 1-3): 두 row가 공유하던 helper 정리.
- **Item 확장**: `recurrenceTextSummary`, `hasReminders`, `streakValueIfRoutine(referenceDate:)` — `Models/Item+Helpers.swift`에 이전. 두 row 모두 1줄 위임으로 축소.
- **`Views/RowHelpers.swift` 신규**: `formatDDay(_:)` free function — "오늘"/"D-N"/"D+N". ItemRow의 "D-day" → "오늘" 라벨로 통일 (MissionRow 시각과 일관).
- **MissionRow 자체 중복**: `valueProgressCapsule` + `progressCapsule` → 단일 `progressCapsule(progress:text:goalColor:monospaceDigit:)`.

### YearGridView 신규 — GitHub contribution graph 스타일
- 새 파일 `Views/YearGridView.swift`. 7 row × 최대 53 col grid.
- firstWeekday 시작, 왼쪽→오른쪽 시간 진행, MonthGridView dot 시각 정책 통일 (목표=원, 할일=round 사각, pending=strokeBorder / completed=filled / cancelled=fill opacity 0.2).
- **strokeBorder 사용** — `stroke()` 대신 frame 외부 spill 없이 fill과 동일 outer size 보장.
- 빈 날짜는 `Color.secondary.opacity(0.05)` (회색, 항목 색 무관) — 활성 일자와 시각 구분.
- 목표 미래일자 → none (사용자 정책: 목표는 과거+오늘만 표시).
- `cellState` MonthGridView.indicatorState 정책 통일 (활동/집중 valueRecorded vs target 직접 검사).
- 가로 swipe → ±1년 (highPriorityGesture로 sheet pan-to-dismiss 충돌 회피).
- ActivityHistoryView per-item monthHeader에 M/Y 토글로 노출 (toolbar trailing 버튼, 현재 상태 표시: month=`calendar` / year=`square.grid.3x3`).

### WeekStripView 제거 + MonthGridView 통합 (week 모드)
- **`Views/WeekStripView.swift` 통째 삭제** (~140줄).
- MonthGridView에 `displayMode: DisplayMode = .month` prop 추가 (`.month` / `.week`).
- `gridStart`/`days`/`weekCount`/`monthAnchor` 모두 mode 분기 — week 모드는 selectedDate 포함 1주 (firstWeekday 시작).
- `onShiftMonth` → **`onShift(Int)`** (의미: ±1 unit — month 모드=월 / week 모드=주). 부모가 해석.
- TodayView D 모드 = `MonthGridView(displayMode: .week, ...)` — dot 인디케이터, picked mode, 카테고리 필터 시각 자동 획득 (기존 WeekStripView는 단순 strip이라 이 정보 없었음).
- **Today 표시 변경**: 일자 숫자 `tint color + semibold` (`numberColor(isToday:inMonth:)`). 빨간 dot 제거. WeekStrip 시각과 통일.

### AddItemView 정리
- **activityHistorySection (preview list) 제거** — 상단 chart 아이콘 entry point와 중복. 8개 helper 함수 (`activityRow`, `activityRowTarget`, `activityRowIconName`, `activityRowIconColor`, `timeText`, `commentText`, `shortDate` + section property) 일괄 삭제 (~120줄).
- **편집 모드 type badge** — 기존 goal 한정 → **편집 모드 모든 항목 노출**. goal은 `kind.goalTypeSymbolName`, Todo는 `checkmark` 사용 (accent fill circle 안에 흰 SF Symbol). 시각 일관성.

### MissionReportView 신규 — 반복 항목 활동 보고서 (Premium 간판 기능)
- 새 파일 `Views/MissionReportView.swift` (~502줄). 반복 항목(목표·반복 Todo)의 통계 보고서.
- **진입점**: AddItemView 상단 보고서 아이콘(`report.title`/`report.show`) → `showFullHistory` sheet → NavigationStack 안 `MissionReportView(item:showsCloseButton:)`. 반복 항목만 노출(1회성은 streak·달성률 의미 X). RC 없어도 미래 occurrence 있는 반복은 노출.
- **구성**: 헤더(아이콘+제목 + [요약][로그] 탭 picker).
  - **요약 탭**: statCard 그리드 — 현재 streak / 최대 streak / (달성 횟수 등) / 달성률(`completionRate`). + 올해 grid(YearGridView 재활용, 정적, swipe 없음).
  - **로그 탭**: 월별 RoutineCompletion 시계열 list (recordRow, 월 전환 shift). ActivityHistoryView 시각 패턴 재사용.
- **Freemium**: 이 보고서가 1.0 Premium(Pro) 간판 3종 중 하나. Settings의 ActivityHistoryView(단순 시계열 기록)는 무료로 유지 — "기록은 무료, 분석/보고서는 Pro" 경계. 자세히는 "Freemium 계획" 섹션 참조.

### ActivityHistoryView recordRow 단순화
- 우측 끝 "Done/Gave up" 상태 라벨 **제거** (leading icon으로 이미 표현 — 중복).
- 별도 라인 comment **제거** → 우측에 통합 (activity/focus는 `current/target`, 그 외는 comment).
- toolbar trailing 순서 재정렬: M/Y → 카테고리/유형 → check 순. 필터 check 마지막.
- 진입 시 `.task`로 `completeFinishedNTDs` + `completeExpiredRoutines` 호출 — sheet 안의 ActivityHistoryView는 RootView.task가 닿지 않아 시간 지난 NTD occurrence가 RC 없이 머무는 케이스 방어.

### MonthGridView UI 폴리시
- **Today 표시**: tint color + semibold (별도 red dot 제거). picker 모드 cell 시각 8pt 공간 고정 유지.
- **하단 padding 제거**: `.padding(.vertical, 6)` → `.padding(.top, 6)` — 마지막 row Divider가 컨테이너 경계와 flush.
- **Picker 모드 + 미선택 cell 빈 공간 처리** — `pickerModeActive` prop으로 분기. true + pickedItemID=nil이면 dot 숨김 + 8pt 빈 공간 유지.
- `categoryFilter`/`goalKindFilter`/`pickedItemID`/`pickerModeActive` 파라미터 순서 재정렬.

### 위젯 폴리시
- leading icon: **circle 배경 + MissionRow 4-state 시각 규칙 적용**.
  - scheduled: secondary outline strokeBorder + secondary icon
  - ongoing: goalColor outline strokeBorder + goalColor icon
  - past: goalColor filled + white icon
  - `widgetLeadingIcon(_:)` helper. frame 22×22pt, 폰트 12pt.
- progressCapsule fix: 안쪽 fill `Rectangle` → `Capsule` (round 유지) + outer `.clipShape(Capsule())` — 작은 진행률에서 round 부분이 영역 바깥으로 나가는 회귀 차단. MissionRow 패턴 통일.

### Tint 풀림 회귀 보강 (UIKit appearance)
- `MyDaysApp.applyTintAppearance()` 추가 — `UIView.appearance().tintColor` + `UIWindow.appearance().tintColor` + 이미 생성된 windows 즉시 갱신.
- `init()`에서 1회 호출 + tintPresetRaw 변경 시 `.onChange`로 재적용.
- SwiftUI `.tint()` propagation이 sheet 깊은 곳(특히 AddItemView idle 시)에서 풀려 시스템 blue로 fallback되는 케이스에 UIKit-level fallback으로 보강. DatePicker, NavigationStack toolbar 등 UIKit-bridge 컴포넌트 정상 색.

### 기타 폴리시
- 활동 기록 필터 라벨: **"Gave up" → "Not completed" / "포기" → "미완료"** (사용자 결정 — `failed`가 NTD 포기뿐 아니라 일반 활동 미달성도 포함하므로).
- MonthGridView picker 모드 + 미선택 = 빈 cell + 8pt 공간 유지 (이전엔 모든 dot 노출).
- 위젯 progress bar Capsule 패턴 통일.

### 다음 (회사 세션 우선순위)
1. 사용자가 시뮬레이터/실기기 시각 확인 후 잔여 회귀 보고.
2. 출시 1.0 로드맵 (CLAUDE.md "다음 작업 분류" 1번)부터 진행 — Firebase Analytics (글로벌 출시 가정 → opt-in 동의 + Settings opt-out + 데이터 삭제 메커니즘).
3. 분리 결정에 따라: iPhone + CloudKit + IAP만으로 출시하면 GDPR 부담 ↓ (Privacy Policy URL + ToS만, ~1일). Firebase 도입 시 풀 GDPR 추가 (~1주). "다음 작업 분류 3번 Firebase 동기화" 섹션의 법적 의무 메모 참조.

## 최근 완료 (2026-06-04 집 세션 2 — 출시 1.0 준비: Privacy Policy + Freemium + Paywall + 알림 fix)

### 출시 1.0 방향 결정
- 1차 출시: **아이폰 / 글로벌 / CloudKit only / 일회성 IAP / 법인 계정**.
- **Firebase Analytics 1.0에서 제외** (opt-in 거부율 높아 데이터 신뢰도 낮음). consent banner/DPA/Analytics 라벨 전부 제거 → Privacy Nutrition Labels "Data Not Collected".
- **동의 게이트 없음** — 수집 안 하므로 명시 동의 불필요. Onboarding 약관 페이지 추가 안 함, 링크만 노출.
- **ToS 자체 작성 안 함** — IAP 일회성이라 Apple EULA 링크 의무도 없음. Privacy Policy 1개만 필수.

### Privacy Policy
- `legal/mydays_privacy_policy.{ko,en}.html` — 자체포함 CSS + 다크모드 + 반응형. 수집 없음 + CloudKit/HealthKit/음성 전부 온디바이스 반영. (md 아닌 HTML, 앱 번들 X — 호스팅 전용)
- 호스팅 URL: `https://app.snapplay.io/docs/mydays_privacy_policy.{ko,en}.html`.
- 앱 내 표시: `Views/SafariView.swift`(SFSafariViewController 래퍼) + `AppLinks.privacyPolicy`(ko/en 분기). Settings [정보] 섹션 "개인정보처리방침" row → 앱 내 Safari.
- **남은 것(사용자)**: HTML placeholder(법인명/주소/privacy@/시행일) 채우기 + 호스팅 업로드 + ASC Privacy Policy URL 입력.

### Privacy Manifest
- `MyDays/PrivacyInfo.xcprivacy` + `MyDaysWidget/PrivacyInfo.xcprivacy` — NSPrivacyTracking=false, 수집/추적도메인 빈 배열, UserDefaults `CA92.1`만 선언(App Group). 서드파티 SDK 없음. objectVersion 77 synchronized folder라 pbxproj 편집 불필요.

### Freemium (1.0 포함) — 자세히는 "Freemium 계획" 섹션
- **일회성 "프로 플랜" 언락** (가격 추후). Premium 3종: **사용량 cap 해제 + 테마 색상 8종 + 활동 보고서(MissionReportView)**. 동기화·검색·활동 기록(ActivityHistoryView)·위젯은 무료.
- 무료 cap: 목표 3 / 반복 3 / 카테고리 3 / 체크리스트 5(할일당). 용어: 전부 **"프로 플랜"**(Pro Plan).
- **`Models/Premium.swift`**: cap 상수 + `canAddGoal/canAddRecurrence/canAddCategory/canAddChecklistItem` + `isUnlocked`(UserDefaults.appShared) + `CapKind`(cap별 메시지) + `ProFeature`(theme/report 안내).
- 게이팅: AddItemView.attemptSave(목표·반복) + appendChecklistDraftAndFocus(체크리스트) + CategoryListView "+"(카테고리) + tintColorChip(테마, Blue만 무료) + 보고서 버튼(AddItemView). 초과/잠금 시 안내 alert.
  - **cap 우회 차단**: 카운트 `status != 2`(보유 전체) — `status==0`만 세면 종료→재활성화로 우회됨(completeExpiredRoutines가 종료일 제거 시 자동 .pending 복원). 반복은 "편집으로 비반복→반복 전환"도 검사(willBeRecurringTodo && !wasRecurringTodo).
- **Paywall**: `Views/PaywallView.swift` — 왕관+혜택4종+구매버튼+가격 placeholder+구매복원+개인정보처리방침. **구매/복원은 테스트용 즉시 언락**(StoreKit 미연결, purchase()/restore() 내부만 교체 예정). alert 4곳에 [프로 플랜으로 업그레이드하기] 버튼 → Paywall sheet. Settings 최상단 상시 진입점. SettingsView Dev 섹션 "프로 플랜 잠금 해제 (테스트)" 토글.
- ⏳ 남음: **StoreKit 2 실제 결제** + ASC 상품 등록 + 가격 확정.

### 알림 버그 fix — CloudKit import 후 재조정
- 증상: 반복 할일(시간없음, 시작일 2시 알림)을 미리 완료했는데 재설치 후 그 occurrence 알림이 fire.
- 원인: `syncNotifications`에 완료 occurrence skip 가드는 있으나, CloudKit이 완료 RC를 **늦게 import**하는 동안(특히 재설치/기기변경) 알림이 먼저 등록됨. remote change는 post 설정만 돼 있고 **구독이 없어** 재조정이 안 됨.
- fix: `RootView`가 `.NSPersistentStoreRemoteChange` 구독(1.5s debounce) → `refreshAllRoutineNotifications`. 재설치/기기변경/다른 기기 완료까지 커버. (RootView에 Combine/CoreData import 추가)

### Settings 동기화 섹션 — 마지막 동기화 시간
- `PersistenceController`가 `NSPersistentCloudKitContainer.eventChangedNotification` 관찰 → import/export 성공 시각을 UserDefaults(`lastSyncDateKey`)에 기록. SettingsView가 @AppStorage로 표시.
- 표시: 1초 미만="방금" / 당일=상대시간("5분 전") / 날짜 넘어감=절대 날짜+시각. 기록 없으면 "아직 없음".

### 다음 (집 → 회사 이어서)
1. 사용자 테스트: Freemium cap/Paywall 흐름, 알림 재조정(실기기+iCloud), 마지막 동기화 표시.
2. **StoreKit 2 결제 연결** (가격 확정 + ASC 상품 등록 필요).
3. Privacy Policy placeholder 채우기 + 호스팅 업로드 + ASC 설정.
4. (보류) alert 버튼 색 차별화 — 시스템 alert 한계(.destructive만 색 변경), 원하면 커스텀 다이얼로그.

## 최근 완료 (2026-06-05 — 락스크린 위젯 분할 원 + 전체 목표 타입)

### SegmentedGoalCircle — 원 1개를 활성 목표 수만큼 자동 분할
- 새 파일 `MyDaysWidget/SegmentedGoalCircle.swift`. 락스크린 위젯 circular/rectangular 공용.
- **자동 분할** (활성 개수 = 분할 수, 최대 4): 1=풀 원 / 2=좌우 / 3=삼각(상·우하·좌하) / 4=사분면.
  - 각 segment = wedge 호(progress trim) + 흐린 트랙(경계 인지) + 아이콘.
  - 커스텀 `WedgeArc` Shape (startDeg/sweepDeg/fraction). 0°=3시, 시계방향(화면 좌표).
  - centroids: 1=[-90], 2=[180,0], 3=[-90,30,150], 4=[-135,-45,45,135](경계 +자 수직·수평).
  - **n=1은 12시(-90°) 시작** 특수 처리 (centroid 공식대로면 6시 시작됨).
  - 아이콘 크기 [22,15,13,11], 반경 factor(지름 대비) [0,0.17,0.21,0.21].
- **락스크린 단색 제약**: 시스템이 틴트 단색 렌더 → 목표별 색 구분 불가. 아이콘+호로만 구분.

### Circular / Rectangular 위젯 재구성
- **Circular** (`MyDaysNTDLockCircleWidget`): entry가 `snapshots: [ItemSnapshot]`(0~4). 상위 최대 4개 분할. 4개 초과면 4개씩 window 회전(`i % windows`).
- **Rectangular** (`MyDaysNTDLockWidget`): entry가 `left/right: [ItemSnapshot]`. `split()` 헬퍼로 **균형 분배**(좌=ceil(n/2)≤4, 우=나머지≤4) → 2=각1개 풀원 / 4=2+2 / 8=4+4. 분할 수 최소화로 가독성 ↑. 최대 8, 초과 시 8개씩 window 회전.
- circular/rect **둘 다 상위(index 0)부터** 표시 — 기존 "circular는 index 2부터(rect와 dedup)" 제거. 단독 위젯에선 이게 맞음(예전엔 circular 단독 시 상위 2개 안 보이던 문제). 두 위젯 같이 두면 상위 겹침 — 수용(사용자 결정). getCurrentConfigurations 기반 dedup은 멀티 잠금화면 오판 등 한계로 미채택.

### 전체 목표 타입 표시 + 우선순위
- 기존: 락 위젯은 절제(NTD) + 활동(auto)만. → **모든 목표(`item.itemKind.isGoal`)** 표시.
- 우선순위(`goalTypeRank`): 절제(0) > 활동 auto(1) > 활동 직접(2) > 집중(3) > 습관(4). 동순위면 bucket(진행중<예정) → sortAnchor.
- 직접입력 활동/집중/습관은 실시간 갱신 X(마지막 진행값), 앱 진입/HK BG 시 reload.

### 버짓
- 분할은 렌더링만 → **타임라인 비용 추가 0**. 오히려 한 번에 여러 개 표시로 rotation entry 감소.
- `transitionInstants`는 NTD만 유지 — 활동/집중/습관은 시간 기반 전환점 없음(값 도달은 비결정적, HK BG reload로 갱신).

### 홈 위젯 분리 — 할일 위젯 / 목표 위젯
- **목적**: 기존 홈 위젯(목표+할일 혼합)을 둘로 분리.
- **할일 위젯** (`MyDaysWidget` 재활용): `makeSnapshots`에서 `item.itemKind == .todo`만 통과 → 할일만. 헤더 카운트도 할일만(scope 목표 카운트 제거). 표시명 "절제 목표/Fast"(레거시) → "할일/Tasks"(widget.ntd.display_name). 디자인 추가 개선은 사용자 피드백 대기.
  - 주의: `MyDaysHomeProvider`의 fetchActiveItems/snapshot 등은 락 위젯·목표 그리드가 공유 → 건드리지 않음. todo 필터는 makeSnapshots(홈 전용)에서만.
- **목표 위젯 신규** (`GoalGridWidget`, small/medium): 락스크린 분할 원(SegmentedGoalCircle) + **색상**.
  - small 2×2(원 4), medium 4×2(원 8). 원당 **최대 2분할** → small 8개 / medium 16개.
  - 데이터: `GoalLockCircleProvider.fetchActiveGoalSnapshots` 재사용(전체 목표 + 우선순위 정렬 + iconColorHex).
  - 분배: `GoalGridEntryView.distribute(_:slots:)` — 균형(n≤slots는 1개씩 풀원, 초과는 앞 원부터 2분할). 상위 우선순위가 앞 원.
  - timeline tier는 홈/락과 동일(NTD transition). entry는 최대 16 snapshot, view가 family별 prefix. **rotation 없음**(16 초과면 상위 16만).
  - 레이아웃: `.contentMarginsDisabled()` + 외곽 padding 16 + 원 사이 spacing 2 + cell당 원 padding 4(원 축소·간격). 시스템 기본 마진(~16) 제거해 원이 가장자리까지.
- **SegmentedGoalCircle 확장**: `colored: Bool` 추가 — true(홈)=목표 색(resolvedColor) / false(락)=widgetAccentable 단색. 호 두께 4→5(공통).
- 번들 등록: MyDaysWidget / GoalGridWidget / ComboWidget / MyDaysLockWidget / MyDaysLockCircleWidget.

### 할일 위젯 디자인 (round box + 시간 라벨)
- 행: **아이콘 제거** → round box(카테고리 색 투명도) + 맨앞 **세로 색상바**(flush, 위아래 full, round box로 clip) + 제목 + 시간 라벨(2번째 줄).
- **시간 라벨**(`ItemSnapshot.timeLabel`, snapshot 시 계산, `MyDaysHomeProvider.todoTimeLabel`):
  - 시간 미지정/기간 중간일 → nil(제목만, 1줄). 단일일+시간 → "시작 - 종료"(시각 같으면 단일). 기간+오늘=시작일 → "시각 시작" / 종료일 → "시각 종료". (모델은 시 단위 → "14:00", 로케일 `jm`)
- **가변 행 높이**: 제목만 24 / 시간 있으면 36. fit은 `TodoLayout.fitted`(높이 budget greedy). 항목 **위부터 정렬**(bottom-align 제거), spacing 4.
- 헤더 카운트: 아이콘 제거 + 좌측 정렬 + "할일 : n/m"(widget.todo.count_format).
- **색상**: 제목 = 라이트=카테고리색 진하게(adjustBrightness -0.34) / 다크=회색 `Color(white:0.82)`. 시간 = 라이트=카테고리색 / 다크=`pastelBright`(채도↓+명도↑, 단순 brightness+는 clamp돼 무효라 채도 down이 핵심). 배경 opacity 라이트 0.11 / 다크 0.24. `Color.adjustBrightness`/`pastelBright` 헬퍼(UIColor HSB).

### 조합 위젯 (ComboWidget, medium/large) — 좌 할일 / 우 목표
- `[.systemMedium, .systemLarge]`. 왼쪽 `TodoColumn`(헤더+할일), 오른쪽 `GoalCirclesView`(목표 원 2열). medium 2×2(8개) / large 2×4(16개). HStack 50/50.
- Provider: todos=`makeSnapshots`, goals=`fetchActiveGoalSnapshots`, counts=`computeCounts`. tier 동일.

### 위젯 컴포넌트 추출 (재사용)
- **할일**: `TodoLayout`(상수/fit) / `TodoHeader` / `TodoRowView` / `TodoEmptyView` / `TodoColumn`(헤더옵션+행, showEmptyState로 오버플로우 컬럼 빈 처리) — MyDaysWidget·ComboWidget 공용.
- **목표**: `GoalCirclesView(snapshots:cols:rows:)` — GoalGridWidget·ComboWidget 공용. distribute(균형 2분할)도 여기로.

### NTD 네이밍 정리
- 락 위젯 파일·타입·kind에서 NTD 제거: `MyDaysNTDLockWidget`→`MyDaysLockWidget`, `MyDaysNTDLockCircleWidget`→`MyDaysLockCircleWidget`(EntryView/kind 포함). (synchronized folder라 pbxproj 무관. 로컬라이즈 키 widget.ntd_lock.*/widget.ntd.*는 유지.)

### 앱 테마 색상 조정 (2026-06-05)
- **기본 tint 변경 blue → coral**. TintPreset enum 순서도 coral 첫번째 / blue 맨뒤(`case coral, peach, ..., blue`) — chip 표시 순서 = 선언 순서. 모든 `@AppStorage` 기본값 + `TintPreset.currentColor` fallback을 `.coral`로 일괄 변경(앱·위젯). (기존 저장값 있는 사용자는 유지.)
- **blue 채도 down**: 시스템 `.blue`(채도 100%) → `Color(0x4A86CF)`(~64%) — 다른 프리셋과 톤 통일.

### 홈 위젯 목표 원 채도 down
- SegmentedGoalCircle `colored`(홈) 모드에서 아이콘·진행 호 색에 `Color.desaturated(0.7)` 적용 — 카테고리 vivid 색이 위젯에서 쨍한 문제. 락(단색)은 무관.

### FocusSessionView 상단 현재 시간
- 집중 화면 상단에 락스크린 느낌 현재 시간 추가 (`TimelineView(.everyMinute)`, 타이머 아님). 날짜(작게) + 오전/오후(작게, 22pt) + 시각(크게, 64pt semibold rounded), 회색 톤(white opacity 0.55), top padding 60.
- 시각/오전·오후 분리(`clockParts`): 12/24h 시스템 설정 반영. ko의 "hmm" 템플릿은 오전/오후를 포함해 중복 → 시각은 고정 포맷 `h:mm`/`H:mm`로 분리. zen 정책(경과/타이머 미표시)은 유지.

## 최근 완료 (2026-06-05 — 세팅탭 재구성 + 피드백)

### Settings 탭 IA 재구성
- 최상위 순서: 프로 플랜 / 앱 색상 / 분류 / 활동 기록 / **도움말 및 피드백** / 동기화 / 정보(→정보 및 권한).
- **활동 기록**: 목표·할일 기록 2개만 (활동 로그는 개발자 메뉴로 이동).
- **도움말 및 피드백** 섹션(활동 기록 바로 아래, 아이콘 없는 텍스트 row): 도움말(준비 중·비활성) / 온보딩 다시 보기 / 버그 신고 및 건의하기.
- **정보** 섹션 → "정보 및 권한" NavigationLink → `InfoPermissionView`(하위 페이지).

### InfoPermissionView (정보 및 권한 하위 페이지)
- 섹션: 권한(앱·건강) / 정보(개인정보처리방침·버전) / 데이터(데이터 내보내기[준비 중·비활성]·전체 데이터 삭제) / **개발자(`#if DEBUG`)**.
- **개발자 섹션 #if DEBUG**: 프로 잠금해제 토글 · 활동 로그 · Pending 알림 · 앱 아이콘 export → **출시 빌드 미노출**. (기존 SettingsView Dev/AppIcon 섹션을 여기로 이동)
- 전체 데이터 삭제 confirm + 건강앱 경로 alert + 개인정보처리방침 SafariView도 이 페이지로 이동.

### 피드백 (이메일)
- `Views/MailComposeView.swift`: `MailComposeView`(MFMailComposeViewController 래퍼) + `FeedbackMail`(recipient `devman@snapplay.io`, subject "[MyDays] 사용자 피드백", body에 버전·iOS·기기 자동 첨부, mailtoURL fallback).
- **버그 신고 + 건의하기 단일 통합** ("버그 신고 및 건의하기"). 계정 있으면 `canSendMail` → 인앱 작성, 없으면 mailto.
- 도움말 화면은 자리만(준비 중) — 추후 구현.

### 데이터 내보내기 (JSON, 무료)
- `Services/DataExportService.exportJSON(in:)` — 모델의 모든 entity를 **제네릭 순회**해 JSON 한 파일로. entity 추가돼도 자동 포함. 관계는 상대 `id(UUID)` 참조로 flatten(향후 import 대비). `{app, schemaVersion(1), exportedAt, entities}`. Date=ISO8601, Data=base64.
- `Views/ShareSheet.swift`(UIActivityViewController 래퍼) — 정보 및 권한 > 데이터 > "데이터 내보내기"(우측 공유 아이콘) 탭 → 파일 `MyDays-backup-yyyyMMdd-HHmmss.json` 생성 → 공유 시트.
- **무료**(데이터 이동권+안전망). 가져오기(import)는 충돌·머지 설계 필요 → 후속.

## 최근 완료 (2026-06-10 — 도움말 웹 + 보고서 네비게이션)

### 도움말 (웹 FAQ + SafariView)
- `legal/mydays_help.{ko,en}.html` — 개인정보처리방침과 같은 스타일(다크모드·반응형) FAQ 10문항 초안 + 스크린샷 placeholder. 호스팅: app.snapplay.io/docs/. (사용자가 placeholder/스크린샷 채워 업로드)
- `AppLinks.help`(ko/en) + Settings > 도움말 및 피드백 > 도움말 → SafariView. (기존 "준비 중" 자리 → 실제 동작)

### MissionReportView 기간 네비게이션
- 공통 `navHeader(title, canNext, prev, next)` — 섹션 헤더 제목 + 우측 **(<)(>) capsule chip**(스테퍼, divider). 다음 버튼은 미래면 disable(흐리게).
- **이번주**(요약): (<)(>) + swipe로 주 이동, 미래 주 disable. 제목 = 이번주면 "이번주", 벗어나면 날짜 범위 "6.2 – 6.8". `weekDate`/`weekForward` 상태.
- **올해**(요약): 우측 연도 라벨 제거 → (<)(>) + swipe 연도 이동, 미래 연도 disable. 제목 = 올해면 "올해" 아니면 "{year}년". `yearValue`/`yearForward`.
- **월간**(로그): 기존 swipe 유지 + (<)(>) 버튼. 월간은 미래 달 허용(스케줄 조회).
- **애니메이션**: MonthGridView/YearGridView에 `animated: Bool` 추가 — 보고서 주/연도는 `animated: false`(즉시 교체, 슬라이드 어색해서 끔). 월간은 슬라이드 유지.

### 활동 목표 달성 알림 정리
- **직접입력(manual)**: 기존엔 알림이 아예 안 떴음(`incrementActivityValue`가 fire 안 함). → 사용자가 직접 채우므로 푸시 무의미 → **알림 토글 숨김(manual은 미노출)** + 달성 시 **success 햅틱**으로 피드백. `incrementActivityValue`는 "새로 달성" 여부 `@discardableResult Bool` 반환.
- **자동수집(HK)**: 알림 토글 유지. 달성 알림은 HK observer(`handleHealthKitBackgroundFire`) 경로로 fire. `fireGoalReachedAlert`는 internal.
- ⚠️ HK 배경 알림은 "앱 실행 시"만 확실 — step/거리 등 누적형은 iOS가 배경 delivery를 throttle(`.immediate`도 ~시간 단위, watch 없으면 지연, 시뮬레이터 거의 안 옴). 코드/entitlement 정상, **Apple 제약**이라 실시간 강제 불가.
...
## 다음 작업 분류 (2026-06-03 — 출시 1.0 로드맵 우선순위 재정렬)

진행 순서:
1. **구글 애널리틱스 (Firebase Analytics) 적용** — 사용자 패턴 측정 인프라 먼저 깔기 (v2 미루지 않고 v1.0에 포함). 자세한 도입 계획은 아래 "v2 작업"에 적힌 Firebase Analytics 블록 참조 (그 블록은 그대로 spec으로 사용, v1.0에 끌어와 실행).
2. **유료기능 unlock 흐름** — Paywall UI 3종 entry + 코드 가드 헬퍼 + DEV unlock 토글. StoreKit 2 통합은 출시 후 v1.x.
   - cap 기준 / Premium 기능 set 확정 — "Freemium 계획" 섹션 참조 (사용자 안 + 권장 보정).
   - Paywall 진입점: Settings 상시 entry / cap 도달 inline 안내 / 기능 진입 시도 시 preview.
3. **Firebase 동기화 — CloudKit ↔ Firebase 양방향 토글**
   - Firestore CRUD wrapper + Auth(Apple/Google) + offline queue + last-writer-wins 충돌 해결.
   - Sync 토글 정책 (CloudKit ↔ Firebase 왕복) — "동기화 토글 정책" 섹션 참조.
   - 이미 prep된 사항: 모든 entity `id: UUID` + `createdAt`/`updatedAt` 일관 갱신 ("Firebase 마이그레이션 prep" 섹션).
   - 작업 추정: Auth + Firestore CRUD wrapper 3~4일 / 동기화 엔진 1.5~2주 / 데이터 마이그레이션·테스트 1주 / 양방향 토글·충돌 해결 추가 1주. 합 **~4~5주**.
   - **Premium 기능으로 묶음** — 유료 unlock 진입점 중 "동기화 ON" 케이스 연결.
   - 1번(Analytics)과 같은 Firebase 프로젝트 재활용 — SDK·plist 중복 작업 없음.
   - **법적 의무 추가 발생** (출시 1.0이 iPhone + CloudKit + IAP만이면 GDPR/약관 부담 낮은데, Firebase 도입 시 데이터가 자체 서버로 가므로 풀 의무 발생):
     - **신규 추가**: 명시 동의 UI(GDPR positive opt-in) / Sign In 약관 동의 화면(이용약관·개인정보처리방침 개별 체크박스) / 계정 관리(export·삭제 — 30일 이내 응답 의무) / Firebase 리전 결정(한 번 정하면 변경 불가).
     - **기존 사용자 처리** (출시 후 Firebase 추가하는 경우): migration prompt 필수 — "동기화 옵션 추가" 안내, Firebase 선택 시 약관 재동의(신규 가입자와 동일 흐름), CloudKit 유지 사용자는 변화 없음.
     - **Privacy Policy 갱신**: Firebase 항목 추가(수집 정보, 보관처, 제3자=Google) + 정책 변경일 명시 + in-app banner로 변경 안내(dismiss 가능).
     - 추가 작업 추정: **~1주** (계정 화면 + 동의 흐름 + Privacy Policy 갱신 + 다국어).
4. **활동 로그 정리** — Settings의 활동 로그 화면 UI 개선 + **로그 데이터 보관 기간 정책** 정의 (예: 90일/1년 자동 cleanup). 출시 후 데이터 누적 안 터지게.
5. **Tutorial & FAQ — Settings 탭에 추가**
   - Tutorial: 주요 기능 단계별 설명. 입력 폼 진입 / 4-type 목표 / 위젯 등.
   - FAQ: 자주 물어볼 만한 질문 (HK 권한 거부 시 / 위젯 안 보임 / 잠금화면 위젯 위치 등).
   - 형태: Settings에 NavigationLink. 본문은 마크다운 정적 텍스트 또는 키-값 list.
   - 다국어 (ko/en) — `Localizable.xcstrings`에 카테고리/항목 추가.
6. **그외 출시 준비** — App Store 자료 + QA + 코드 정리. 자세히는 "출시 1.0 직전" 섹션 참조.

부수 작업 (소형 폴리시, 진행 사이사이):
- 홈 위젯 large 사이즈 추가 검토 — small/medium 패턴 확장.
- tint 풀림 이슈 — 재현 시 그때 대응.

### 빈 화면 작업 상태 (완료 — 2026-06-02 회사 세션 마감)

**최종 정책 (Case A/C 통합 + B 분리)**:
- **활성 0 케이스** (필터 없음): 모두 같은 EmptyStateView UI 사용. List 안 첫 Section의 row로 배치 → 스크롤 시 같이 올라가 완료 섹션 공간 확보.
  - 메시지·아이콘은 first-launch 톤 통일 (활성 0이면 완료 유무 무관 동일 메시지).
  - 토글 ON + finished 1+: 큰 메시지 위 + finished row 섹션 아래 동시 노출.
  - 토글 OFF: 큰 메시지만 (섹션 자체 hide).
- **Case B (필터/검색 결과 empty)**: 별도 inline `filteredEmptyRow` (작은 필터 아이콘 + 메시지 + "필터 해제" 버튼).
- **Case D (Picker mode + occurrence 없음)**: 후순위 (보류).

**EmptyStateView** (`Views/EmptyStateView.swift`):
- params: `iconName`, `message`, `hint?`(optional secondary), `alignment` (.center/.top), `topPadding` (`.top` 모드 위쪽 padding, default 140).
- `.top` 모드: 자기 콘텐츠 크기 + 위쪽 padding (호출 측 상단 UI에 따라 조정). List row 안에 두면 listRow modifier(`listRowBackground/Separator/Insets`)로 자연 통합.
- "(+)" 마커: main/hint 둘 다 자동 감지 → `plus.circle.fill` inline 치환.

**탭별 구현 위치 + topPadding**:
- **ArchiveView**: 활성 0 (`!hasFilter && items.isEmpty`) → `archiveEmptyLayout` (List 안 Section의 `EmptyStateView(tray.full.fill, archive.empty.first)` + 완료 섹션 토글 ON 시). `topPadding: 140`.
- **ListView**: 활성 0 (`!hasFilter && sortedActiveItems.isEmpty`) → `emptyLayout` (List 안 Section의 `EmptyStateView(note.text, list.empty.first)` + 완료 섹션 토글 ON 시). `topPadding: 140`.
- **TodayView**: `isGenuineEmpty` = `activeNtds.isEmpty && activeActivities.isEmpty` (finished 제외 활성 기준). 활성 0이면 List 첫 Section에 `EmptyStateView(N.calendar, today.empty.first/.first.other, hint: today.empty.add_hint)`. Mission/Todo 섹션은 빈 채면 hide(중복 회피), 활성 1+ 케이스는 기존 emptyRow 유지.
  - `topPadding: viewMode == .month ? 0 : 40` (상단 WeekStrip/MonthGrid 있어 작게).
  - hint: 과거 일자면 hide (오늘/미래만 "(+) 를 눌러 새로운 계획을 추가할 수 있어요." 노출).
  - 아이콘: viewed date의 day 동적 (Calendar.gmt).

**Case B 인라인 (필터 결과 0)**:
- ListView/ArchiveView: `filteredEmptyRow` 헬퍼 — 작은 `line.3.horizontal.decrease.circle` tertiary + 메시지 + "필터 해제" 버튼(filterCategoryID/filterGoalKind = nil).
- ListView 검색 모드: 기존 emptyState 그대로 (`list.search.no_results`).
- TodayView: 기존 작은 emptyRow 유지.

**Localization 키 추가**:
- `today.empty.first` / `today.empty.first.other`: "남은 목표나 할일이 없어요." (활성 기준 톤, "first/첫" 제거).
- `today.empty.add_hint`: "(+) 를 눌러 새로운 계획을 추가할 수 있어요."
- `list.empty.first`: "(+) 를 눌러 할일이나 목표를 추가해보세요." ("first/첫" 제거).
- `list.empty.filtered`, `archive.empty.filtered`: "이 필터와 일치하는 항목이 없어요."
- `filter.clear`: "필터 해제" / "Clear filter".

**탭 아이콘**: 목록 `list.bullet` → `note.text`, 보관함 `archivebox` → `tray.full.fill` (탭 아이콘과 EmptyStateView 아이콘 통일로 컨텍스트 식별 강화).
...
### v2 작업
- **무료/유료 cap 실제 활성화** — StoreKit 2 통합 + paywall 결제 흐름. (v1.0에서 cap 가드 + paywall UI까지는 깔고, v2에서 StoreKit 연결 + 실제 unlock.)
- **구글 애널리틱스 (Firebase Analytics)** — **v1.0 출시 1순위 작업으로 이동** (위 "다음 작업 분류" 참조). 아래 블록은 도입 spec — 그대로 v1.0에 끌어와 실행.
  - **SDK**: Firebase Analytics (= GA for Firebase). 독립 GA SDK는 deprecated. SPM으로 `FirebaseAnalytics`만. Crashlytics는 옵션이지만 출시 안정성용으로 같이 도입 권장. 위젯 extension엔 SDK 미포함 (위젯 process에서 호출 안 함 정책).
  - **Firebase 프로젝트**: 동기화 prep과 같은 프로젝트 재활용 → 도입 비용 50% 절약. iOS app bundle `io.snapplay.MyDays` 등록 + `GoogleService-Info.plist` Xcode root에 추가.
  - **법적 / 개인정보 (출시 차단 요소)**:
    - `PrivacyInfo.xcprivacy` — iOS 17+ 필수. Firebase 공식 템플릿 + 앱 자체 reasons 추가.
    - **ATT prompt 안 띄우는 게 권장** — Firebase Analytics는 IDFA 없이 충분. ATT 띄우면 거부율 70%+ → 어차피 데이터 손실.
    - App Store Privacy Nutrition Labels: "Usage Data", "Diagnostics" 선언 (출시 준비 TODO와 같이 처리).
    - PIPA(한국) / GDPR(EU) 동의 prompt 검토 — 묵시적 vs 명시적. 분석 목적은 명시 동의 권장.
  - **AnalyticsService wrapper** (`Services/AnalyticsService.swift`) — Firebase SDK 호출 캡슐화 + enum 기반 event (type-safe). 향후 SDK 교체/끄기 용이.
    ```
    AnalyticsService.shared.log(.itemCreated(kind: .activity, hasRecurrence: true))
    ```
  - **Event 분류**:
    - **자동**: app_open / screen_view / first_open / session_start.
    - **커스텀**: `item_created` (kind/has_recurrence/has_reminder) / `item_completed` (kind/via=manual·auto·hk) / `mission_failed` (kind/reason_type) / `focus_started` / `focus_completed` / `focus_aborted` (duration_min/reason) / `widget_tapped` (family) / `onboarding_completed` / `onboarding_skipped` (page_reached) / `paywall_shown` / `paywall_purchased` / `paywall_dismissed` (entry_point) / `hk_sync_failed` (source).
  - **User Properties**: `total_active_items` (100단위 bucket) / `has_recurring_routines` / `is_premium` (freemium 도입 후) / `appearance_mode` / `tint_preset`.
  - **코드 통합 위치**:
    - App init: `MyDaysApp.init()`에서 `FirebaseApp.configure()`.
    - 액션 시점: `AddItemView.save()`, `Item.toggleDone`/`cancel`, `FocusSessionManager.stopSession`, `Item.incrementActivityValue`, NTD 포기(NTDRow.giveUp), 위젯 tap, paywall flow.
    - 화면 진입: 수동 `.onAppear`에서 screen_view 호출 (NavigationDestination에서).
  - **개발/운영 분리**: `#if DEBUG`로 dev 빌드는 local console만 / prod만 Firebase 전송. 또는 Firebase DebugView 활성화(`-FIRDebugEnabled`)로 콘솔에서 실시간 event 확인. TestFlight 빌드는 별도 Firebase 프로젝트(dev) 권장.
  - **도입 순서**: (1) Firebase 프로젝트 등록 → (2) SDK + plist → (3) PrivacyInfo.xcprivacy → (4) AnalyticsService wrapper + event enum → (5) 액션 위치별 log 삽입 → (6) PIPA/GDPR 동의 UI (옵션) → (7) Nutrition Labels 갱신 → (8) 출시 후 funnel/retention 대시보드.
- **Firebase 동기화** — **v1.0 출시 3순위 작업으로 이동** (위 "다음 작업 분류" 참조). spec: CloudKit ↔ Firebase 양방향 토글 + last-writer-wins. 로그인 화면 (Apple/Google). 자세한 정책은 "동기화 토글 정책" / "Firebase 마이그레이션 prep" 섹션.
- **세팅탭 기능 정리** — 권한 / 데이터 / 동기화 / 보고서 / 백업 등 재구성.
- **데이터 export/import** — JSON 백업. Freemium 데이터 안전망 + 동기화 변경 사이 fallback.
- **구글 캘린더 단방향 내보내기 (Pro 후보)** — 양방향 풀 동기화는 비추(난이도 上 ~3~4주 + 완료/목표/반복-회차가 캘린더에 안 맞음). **단방향 export**만 (난이도 中상 ~1~1.5주).
  - 매핑: dated **할일만** → Google Calendar Event (title=summary, notes=description, start/dueHour=date/dateTime, recurrenceRule→RRULE, reminders→overrides). 목표/체크리스트/streak은 대응 없음 → 제외.
  - ⚠️ **완료(done)는 캘린더에 대응 필드 없음** — Event.status는 confirmed/tentative/cancelled(완료 아님). 옵션: extendedProperties.private(UI 안 보임) / 제목 ✓ prefix / Google **Tasks API**(완료 있음, 시간·반복 약함). 완료 매칭 중요하면 Tasks 검토.
  - **upsert(중복 방지)**: Item에 `googleEventId`(+`googleCalendarId`) 추가 → 있으면 events.update, 없으면 insert. **CloudKit 동기 모델에 저장**하면 크로스기기 중복 방지. 엣지: 구글측 삭제(404→재insert) / 앱 삭제(events.delete) / 토큰 refresh. (deterministic id = item.id base32hex 대안 가능)
  - OAuth(Google Sign-In) + Calendar API. Pro 기능으로 묶기 좋음.
...
## Freemium 계획 (2026-06-04 확정 — 1.0에 포함)

### ✅ 확정 스펙 (2026-06-04)
- **모델**: 일회성 "MyDays Pro" 언락 (구독 아님). **가격은 추후 결정** (코드는 가격 무관 진행, ASC에서 출시 전 설정).
- **Premium (Pro) 3종**:
  1. 사용량 cap 해제 (무제한)
  2. 테마 색상 8종 (TintPreset)
  3. **활동 보고서 = MissionReportView만** (요약 통계 + YearGrid + 로그)
- **무료 (Pro 아님)**: 동기화(CloudKit) / 활동 검색+#태그 / **활동 기록(ActivityHistoryView, Settings)** / 위젯 — 전부 무료. 경계: "기록은 무료, 분석/보고서는 Pro".
- **무료 Cap** (조정 가능):
  | 항목 | 무료 | Pro |
  |---|---|---|
  | 목표(4-type 합계, 활성) | 3 | ∞ |
  ...
  | 카테고리 | 3 | ∞ |
  | 체크리스트/할일 | 5 | ∞ |
- **Paywall**: 1.0에 포함. 단 StoreKit 2 결제 연결은 **나중**. 먼저 **가드 헬퍼 + Dev 언락 토글**로 cap 동작 테스트부터 (paywall UI/결제 후속).
- **게이팅 위치**: canAddGoal / canAddRecurrence / canAddCategory / canAddChecklistItem 가드 헬퍼 + 테마 chip 잠금 + MissionReportView 진입 잠금. `isPremium` = StoreKit transaction + `@AppStorage(store: .appShared)` 캐시(위젯 인지) + Settings Dev unlock 토글.
